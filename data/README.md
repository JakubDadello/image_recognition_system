# Data Folder

This directory is intended to hold the project datasets. Due to file size limits, the raw data is not included in the repository.

## Contents
- `dataset.zip`: Original Metallic Surface Defect Detector compressed data.
- `steel_data/`: Extracted images organized into subfolders per class.

## Data Source
The dataset used in this project is sourced from Roboflow:
[Metallic Surface Defect Detector](https://universe.roboflow.com/sujitsa/metallic-surface-defect-detector/dataset/1)
